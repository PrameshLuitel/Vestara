...
export default function PredictiveSuiteLegacy() {
            ...
            <CardContent className="p-0">
                 {loading && !allData ? (
                    ...
                    <Alert variant="destructive" className="bg-destructive/20 border-destructive/50 text-destructive-foreground">
                      ...
                      <AlertDescription>
                        {error}
                         {error.includes("NEXT_PUBLIC_GOOGLE_API_KEY") && (
                            <p className="mt-2 text-xs">Please ensure your Google API Key is correctly configured in your environment variables.</p>
                         )}
                        </AlertDescription>
                    </Alert>
                 ) : currentStockData ? (
